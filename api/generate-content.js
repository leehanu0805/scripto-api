...
module.exports = async (req, res) => {
  ...
  const WPS_TABLE = {
    ...
    ja: Number(process.env.WPS_JA) || 3.2, // pseudo-words/sec (문자 기반)
    zh: Number(process.env.WPS_ZH) || 3.2, // pseudo-words/sec
...
'
  ...
  function wordWeight(line, lang) {
    ...
    // 한국어/일어/중국어는 약간 더 빠르게 읽히니 가중치 미세 조정
    if (L === "ko" || L === "ja" || L === "zh") return Math.max(1, Math.round(words * 1.05));
  ...
  function retimeScript(script, totalSec, lang) {
    try {
      ...
      // HOOK/CTA 제약
      const hookMin = 0.10 * T, hookMax = 0.15 * T;
